...
class SalesManagement:
    ...
    def check_if_customer_exist(self, customer_id):
        customer_ids = self.customer.customers[0]
    ...
    def main(self):
        ...
        while True:
            ...
            print("8. Load customer records from a CSV")
            print("9. Save customer records to a CSV")
            print("10. Load transaction records from a CSV")
            print("11. Save transaction records to a CSV")
            print("12. Display Monthly Sales & Transactions")
            print("13. Display Customer Monthly Sales & Transactions")
            print("14. Display Postcode Monthly Sales & Transactions")
            print("15. Quit \n")
            ...
            if choice == '1':
                ...
                self.customer.load_customer_from_csv(csv_file)

            elif choice == '9':
                if len(self.customer.customers) == 0:
                    print("No customer records to export. \n")
                else:
                    csv_file = validate_csv_file()
                    self.customer.save_customer_to_csv(csv_file)

            elif choice == '10':
                csv_file = validate_csv_file()
                if len(self.customer.customers) > 0:
                    customer_ids = set(self.customer.customers[:, 0])
                else:
                    customer_ids = set()

                self.transaction.load_transaction_from_csv(customer_ids, csv_file)

            elif choice == '11':
                if len(self.transaction.transactions) == 0:
                    print("No transaction records to export. \n")
                else:
                    csv_file = validate_csv_file()
                    self.transaction.save_transaction_to_csv(csv_file)

            elif choice == '12':
                self.transaction.plot_monthly_sales_and_transactions()

            elif choice == '13':
                self.transaction.plot_customer_monthly_sales_and_transactions()

            elif choice == '14':
                self.transaction.plot_postcode_monthly_sales_and_transactions(self.customer.customers)
